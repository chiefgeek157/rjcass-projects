...
public class TripleDESByte
{
	...
	public static final int			PERMUTED_KEY_SIZE_BYTES	= 7;
	...
	//
	// BlockGeneratorByte bg = new BlockGeneratorByte(is);
	...
	public void encrypt(InputStream is, byte[] keyGroup, OutputStream os) throws IOException
	{
		...
		byte[] block1 = new byte[BLOCK_SIZE_BYTES];
		byte[] block2 = new byte[BLOCK_SIZE_BYTES];

		int blockNum = 0;
		int numBlocks = fillBuffer(is, buf);
		while (numBlocks > 0)
		{
			while (blockNum < numBlocks)
			{
				// System.out.println("BLOCK " + blockNum + ": "
				// + BitPrinter.toString(buf, blockNum * BLOCK_SIZE_BYTES,
				// (blockNum + 1) * BLOCK_SIZE_BYTES));
				processBlock(false, keyGroup, 0 * KEY_SIZE_BYTES, buf, blockNum * BLOCK_SIZE_BYTES, block1);
				processBlock(true, keyGroup, 1 * KEY_SIZE_BYTES, block1, 0, block2);
				processBlock(false, keyGroup, 1 * KEY_SIZE_BYTES, block2, 0, block1);
				// BlockGenerator.writeBlock(o3, BLOCK_SIZE_BITS, os);
				blockNum++;
			}

			numBlocks = fillBuffer(is, buf);
			blockNum = 0;
		}
	}
	...
	// throw new IllegalArgumentException("Key group cannot be null");
	// if (keyGroup.length != KEY_GROUP_SIZE_BYTES)
	// throw new IllegalArgumentException("Key group must be " +
	// KEY_GROUP_SIZE_BYTES + " bytes");
	// if (is == null)
	// throw new IllegalArgumentException("Input stream cannot be null");
	// if (os == null)
	// throw new IllegalArgumentException("Output stream cannot be null");
	...
	private int fillBuffer(InputStream is, byte[] buf) throws IOException
	{
		int numRead = is.read(buf);
		// System.out.println("Bytes read: " + numRead);
		int numBlocks = (int)Math.ceil((double)numRead / BLOCK_SIZE_BYTES);
		// System.out.println("Num blocks: " + numBlocks);

		if (numRead > 0)
		{
			int gap = numBlocks * BLOCK_SIZE_BYTES - numRead;
	...
	private void processBlock(boolean decrypt, byte[] keys, int keyOffset, byte[] buf, int blockOffset, byte[] outBlock)
	{
		System.out.println("Key  : " + BitPrinter.toString(keys, keyOffset, keyOffset + KEY_SIZE_BYTES));
		System.out.println("Block: " + BitPrinter.toString(buf, blockOffset, blockOffset + BLOCK_SIZE_BYTES));

		byte[] workingBlock = new byte[BLOCK_SIZE_BYTES];
		permute(buf, blockOffset, INITIAL_PERMUTATION, BLOCK_SIZE_BITS, workingBlock);

		System.out.println("   Initial Permutation: " + BitPrinter.toString(workingBlock));

		byte[] permutedKey = new byte[PERMUTED_KEY_SIZE_BYTES];
		permute(keys, keyOffset, PERMUTED_CHOICE_1, PERMUTED_KEY_SIZE_BITS * 2, permutedKey);

		for (int i = 0; i < NUM_PERMUTED_KEYS; i++)
		{
			 leftShift(cd0[0], PERMUTED_KEY_LEFT_SHIFT[i], PERMUTED_KEY_SIZE_BITS,
			 cd1[0]);
			 leftShift(cd0[1], PERMUTED_KEY_LEFT_SHIFT[i], PERMUTED_KEY_SIZE_BITS,
			 cd1[1]);
			//
	...
	// // BitPrinter.toString(eBlock, CIPHER_SIZE_BITS, 6));
	//
	// BitSet[] cipherBlocks = split(eBlock, CIPHER_SIZE_BITS,
	// CIPHER_BLOCK_SIZE_BITS);
	//
	...
	// private byte[][] getPermutedKeys(byte[] keys, int offset)
	// {
	// byte[][] permutedKeys = new
	// byte[NUM_PERMUTED_KEYS][PERMUTED_KEY_SIZE_BYTES];
	//
	// byte[] pk = new byte[PERMUTED_KEY_SIZE_BYTES];
	// permute(keys[offset], PERMUTED_CHOICE_1, PERMUTED_KEY_SIZE_BITS * 2, pk);
	//
	// BitSet[] cd0 = split(pk, PERMUTED_KEY_SIZE_BITS * 2,
	// PERMUTED_KEY_SIZE_BITS);
	//
	...
	private void permute(byte[] buf, int offset, int[] permutation, int permutationNumBits, byte[] permuted)
	{
		for (int i = 0; i < permutationNumBits; i++)
		{
			int srcByteNum = permutation[i] / Byte.SIZE;
			int srcBitNum = permutation[i] % Byte.SIZE;
			int srcBit = 0x1 << srcBitNum;

			int tgtByte = i / Byte.SIZE;
			int tgtBitNum = i % Byte.SIZE;
			int tgtBit = 0x1 << tgtBitNum;

			if ((buf[offset + srcByteNum] & srcBit) == srcBit)
				permuted[tgtByte] |= tgtBit;
			else
				permuted[tgtByte] &= ~tgtBit;
		}
	}
